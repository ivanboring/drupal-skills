...
start() {
  ...
  # No `$!` PID capture: the container runs bash with `set -u` (nounset via SHELLOPTS/BASH_ENV),
  # which trips on `$!`. stop() uses `pkill -x chromium`, so no PID file is needed.
...
--disable-features=Translate --user-data-dir=/tmp/tut-chrome"
  ...
  # ddev "Failed to execute command" noise from those polls.
  local i
