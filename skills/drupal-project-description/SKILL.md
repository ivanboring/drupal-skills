...
into the project's description field. It also knows the special CSS classes drupal.org ships that
make a page pop: checkered note boxes, tip boxes, green buttons, grids, and side-by-side listings.
...
The description should read like a maintainer wrote it, not like an AI generated it. Apply these
rules to all prose in the description.

- No em dashes and no en dashes anywhere. Use a comma, semicolon, colon, period, or parentheses,
  or rewrite the sentence. Plain hyphens are fine only in compound words (e.g. site-builder).
- Active voice.
...
- No rhetorical negation such as "not optional, it's required." Just state the point.
- No marketing hype, subjective qualifiers, or value judgments. State facts a user can verify.
- No emojis or icons in the prose.
- Keep it terse and high in signal. Cut padding and throat-clearing intros.
...
2. **Editing an existing page?** Read the live page first. The data name is the last URL segment
   (`drupal.org/project/ai` gives `ai`):
   ```bash
...
3. **New project?** Include these headings (as `<h2>`), in this order, skipping any that don't apply:
   - **Features**
   - **Installation**
   - **Related Modules**, only if any exist. Find them by searching the module list at
     `https://www.drupal.org/project/project_module` for modules similar to this one (same job or
     alternatives), not modules that share its ecosystem or project family. Use an `<h3>` for this
     heading, not an `<h2>`, and keep the heading outside any special div. A special div (such as
     `view-book-listings`) can follow underneath the `<h3>`, never wrapping it.
...
Headings `<h2>` to `<h4>`, `<p>`, `<strong>`, `<em>`, `<i>` (cursive), `<code>`, `<blockquote>`,
...
### Book listing: heading plus intro, a rule, then side-by-side (50%) links
Each `<li>` in `ul.guide-contents` renders at half width, so links or items sit two per row.
```html
...
| Prose that reads like AI (em dashes, hype, contrastive phrasing) | Follow the Writing Style rules above. |
